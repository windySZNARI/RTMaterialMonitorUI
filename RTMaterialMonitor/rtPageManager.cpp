...
UA_StatusCode rtPageManager::writeDate(const char *pValue, int nNodeListSize, const char *pItem, uint16_t nUaTypes)
{
	printf("write data\n");
	int iRet = 0;
...
void rtPageManager::getObjectList()
{
	// Browse some objects
...
void rtPageManager::clientClose()
{
	m_bConnected = false;
	m_mapBrowse.clear();
...
void rtPageManager::addNode(char *pNodeName, uint16_t nUaTypes, char *pBrowseName)
{
	printf("add new node\n");
	UA_NodeId var_id;
...
void rtPageManager::readThread()
{
	...
	while (1)
	{
		...
		{		
			...
			emit signalSendData(itemName, strContent);
		}
		else
		{
			printf("读数据失败\n");
